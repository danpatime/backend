...
public interface ContractRepository extends JpaRepository<Contract, Long> {
    ...
    @Query("select new com.example.api.suggest.controller.dto.request.OfferEmploymentDetailRequest(e.name, b.businessName, oe.status, c.contractHourlyPay, c.contractStartTime, c.contractEndTime, cr.chatRoomId, c.contractId) " +
            "from Contract c " +
            "join c.offerEmployment oe " +
    ...
    @Query("select new com.example.api.review.dto." +
            "ReviewAvailableResponse(c.contractId, e.accountId, e.name) " +
            "from Contract c " +
            "join c.offerEmployment oe " +
            "join oe.employee e " +
            "join oe.business b " +
